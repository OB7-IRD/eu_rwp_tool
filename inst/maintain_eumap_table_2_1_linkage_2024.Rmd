...
Linkage file from 2023. This file is use as inputs to all the ammendments made in 2024
...
Linkage file use to create table 2.1 the 30th of May.

Main improvements:

1.  Checking and correcting quotas

2.  Fixing overlapping stocks
...
linkage$fides_area <- gsub("noTAC", "NoTAC", linkage$fides_area)
...
linkage_fides_areas <- unlist(x = strsplit(x = as.character(x = linkage$fides_area),
                                        split = ","))
...
for (i in 1:nrow(linkage)) {
  ...
  areas <- data.frame(area = unlist(x = strsplit(x = as.character(x = linkage$fides_area[i]),
                                        split = ",")))
  
  species <- data.frame(species = unlist(x = strsplit(x = as.character(x = linkage$x3a_code[i]),
                                        split = ",")))
